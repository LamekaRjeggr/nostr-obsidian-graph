...
var DEFAULT_SETTINGS = {
  ...
  // seconds
  pubkey: ""
  // Will be set by user in settings
};

// src/modals/search-modal.ts
var import_obsidian = require("obsidian");
var SearchModal = class extends import_obsidian.Modal {
  constructor(app, vaultService, onSearch) {
    super(app);
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName("Keyword").setDesc("Enter search term").addText((text) => text.setPlaceholder("Enter keyword...").setValue(this.keyword).onChange((value) => this.keyword = value));
    new import_obsidian.Setting(contentEl).addButton((btn) => btn.setButtonText("Search").setCta().onClick(async () => {
      if (!this.keyword || this.searching)
        return;
      this.searching = true;
      ...
      try {
        ...
        this.close();
        new import_obsidian.Notice(`Found ${results.length} notes`);
      } catch (error) {
...
// src/services/core/pool.service.ts
var PoolService = class {
  constructor() {
    this.pool = new SimplePool();
  }
  sub(relayUrls, filters) {
    let eventCallback = null;
    let eoseCallback = null;
...
// src/services/core/relay.service.ts
...
// src/services/core/event-emitter.ts
...
// src/services/core/event.service.ts
var EventService = class {
  ...
  async subscribe(filter, subscriptionId) {
...
// src/services/core/key.service.ts
var KeyService = class {
  async npubToHex(npub) {
    ...
    try {
      const { data: hex2 } = nip19_exports.decode(npub);
      return hex2;
    } catch (error) {
      throw new Error("Invalid npub format");
    }
  }
  async hexToNpub(hex2) {
    if (!/^[0-9a-f]{64}$/i.test(hex2)) {
      throw new Error("Invalid hex format");
    }
    try {
      return nip19_exports.npubEncode(hex2);
    } catch (error) {
      throw new Error("Invalid hex format");
    }
  }
...
// src/services/obsidian/metadata-cache.service.ts
var MetadataCacheService = class {
  constructor(app) {
    this.app = app;
  ...
  updateCache(file) {
    ...
    if (file.path.startsWith("nostr/")) {
      ...
      if ((_a = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a.event) {
   ...
   */
  async findEventFile(eventId) {
    var _a, _b;
    for (const [path, cache] of this.metadataCache.entries()) {
      if (((_b = (_a = cache.frontmatter) == null ? void 0 : _a.event) == null ? void 0 : _b.id) === eventId) {
   ...
   */
  async findEventsByAuthor(pubkey) {
    var _a, _b;
    const results = [];
    for (const [path, cache] of this.metadataCache.entries()) {
      if (((_b = (_a = cache.frontmatter) == null ? void 0 : _a.event) == null ? void 0 : _b.pubkey) === pubkey) {
   ...
   */
  async findEventsByKind(kind) {
    var _a, _b;
    const results = [];
    for (const [path, cache] of this.metadataCache.entries()) {
      if (((_b = (_a = cache.frontmatter) == null ? void 0 : _a.event) == null ? void 0 : _b.kind) === kind) {
        const file = this.fileCache.get(path);
        if (file)
          results.push(file);
      }
   ...
   */
  async getEventsFromFiles(files) {
    return files.map((file) => {
      var _a, _b;
      return (_b = (_a = this.metadataCache.get(file.path)) == null ? void 0 : _a.frontmatter) == null ? void 0 : _b.event;
...
// src/services/obsidian/link.service.ts
...
var LinkService = class {
   ...
   */
  async getReferencedProfiles(noteId) {
    ...
    if (!(cache == null ? void 0 : cache.links))
      return [];
    return cache.links.filter((link) => link.link.startsWith("nostr/user profile/")).map((link) => link.link.replace("nostr/user profile/", "").replace(".md", ""));
  }
  /**
   * Get notes that reference a profile
   */
  async getProfileMentions(profileId) {
    const profileFile = this.app.vault.getAbstractFileByPath(`nostr/user profile/${profileId}.md`);
    if (!(profileFile instanceof import_obsidian2.TFile))
      return [];
    const backlinks = this.app.metadataCache.getBacklinksForFile(profileFile);
    return Array.from(backlinks.data.keys()).filter((path) => typeof path === "string" && path.startsWith("nostr/user notes/")).map((path) => path.replace("nostr/user notes/", "").replace(".md", ""));
  }
  /**
   * Get notes referenced by another note
   */
  async getReferencedNotes(noteId) {
    const file = this.app.vault.getAbstractFileByPath(`nostr/user notes/${noteId}.md`);
    if (!(file instanceof import_obsidian2.TFile))
      return [];
    const cache = this.app.metadataCache.getFileCache(file);
    if (!(cache == null ? void 0 : cache.links))
      return [];
    return cache.links.filter((link) => link.link.startsWith("nostr/user notes/")).map((link) => link.link.replace("nostr/user notes/", "").replace(".md", ""));
  }
  /**
   * Update links when saving a new event
   */
  async updateLinks(event) {
    if (event.kind === 1) {
      const profileRefs = event.tags.filter((tag) => tag[0] === "p").map((tag) => tag[1]);
      const noteRefs = event.tags.filter((tag) => tag[0] === "e").map((tag) => tag[1]);
      const links = [
  ...
  async processNoteLinks(file, cache) {
    const profileRefs = cache.links.filter((link) => link.link.startsWith("nostr/user profile/")).map((link) => link.link.replace("nostr/user profile/", "").replace(".md", ""));
    const noteRefs = cache.links.filter((link) => link.link.startsWith("nostr/user notes/")).map((link) => link.link.replace("nostr/user notes/", "").replace(".md", ""));
...
---`;
  ...
  async processProfileLinks(file, cache) {
    const backlinks = this.app.metadataCache.getBacklinksForFile(file);
    const mentioningNotes = Array.from(backlinks.data.keys()).filter((path) => typeof path === "string" && path.startsWith("nostr/user notes/")).map((path) => path.replace("nostr/user notes/", "").replace(".md", ""));
...
// src/services/nostr/profile.service.ts
var ProfileService = class {
  constructor(relayService, eventService) {
    this.relayService = relayService;
    this.eventService = eventService;
  }
  async fetchEvent(pubkey) {
    await this.relayService.ensureConnected();
...
// src/services/nostr/note.service.ts
...
// src/services/nostr/follow.service.ts
...
// src/services/obsidian/vault.service.ts
var VaultService = class {
  constructor(app, indexService, obsidianFileService, profileFileService, noteFileService, followFileService) {
    this.app = app;
    this.indexService = indexService;
    this.obsidianFileService = obsidianFileService;
    this.profileFileService = profileFileService;
    this.noteFileService = noteFileService;
    this.followFileService = followFileService;
    this.baseDir = "nostr";
    this.notesDir = "nostr/user notes";
    this.profilesDir = "nostr/user profile";
    this.followsDir = "nostr/user follows";
    this.followedProfilesDir = "nostr/followed profiles";
  }
  async saveEvent(event, isFollowedProfile = false) {
    switch (event.kind) {
      case 0:
...
var ObsidianFileService = class {
  ...
  createFrontmatter(data) {
    ...
    const yaml = Object.entries(yamlData).map(([key, value]) => {
      if (Array.isArray(value)) {
        return `${key}:
${value.map((v) => `  - ${v}`).join("\n")}`;
      }
...
// src/services/core/frontmatter.service.ts
var FrontmatterService = class {
  static createBaseFrontmatter(event) {
    return {
      created: new Date(event.created_at * 1e3).toISOString(),
      kind: event.kind,
      id: event.id,
      pubkey: event.pubkey,
  ...
  static createEventFromFrontmatter(frontmatter, content) {
    return {
      id: frontmatter.id,
      pubkey: frontmatter.pubkey,
      kind: frontmatter.kind,
      created_at: new Date(frontmatter.created).getTime() / 1e3,
...
// src/services/obsidian/profile-file.service.ts
var ProfileFileService = class {
  constructor(obsidianFileService, indexService) {
    this.obsidianFileService = obsidianFileService;
    this.indexService = indexService;
    this.defaultProfileDir = "nostr/user profile";
  }
  async saveProfile(event, directory) {
    if (event.kind !== 0) {
  ...
  async getProfile(pubkey) {
    ...
    for (const file of files) {
      ...
      if ((frontmatter == null ? void 0 : frontmatter.kind) === 0) {
...
// src/services/core/filename.service.ts
var FilenameService = class {
  /**
   ...
   */
  static generateNoteFilename(event) {
    if (event.kind !== 1) {
      throw new Error("generateNoteFilename can only be used with kind 1 events");
    }
    return event.content.split(/[.!?](?:\s|$)/)[0].trim().toLowerCase().replace(/[^a-z0-9]+/g, " ").trim().replace(/\s+/g, " ").slice(0, 40);
  }
};

// src/services/obsidian/note-file.service.ts
var NoteFileService = class {
  constructor(obsidianFileService, indexService) {
    this.obsidianFileService = obsidianFileService;
    this.indexService = indexService;
    this.notesDir = "nostr/user notes";
  ...
  async saveNote(event) {
    ...
    try {
      ...
      const frontmatter = FrontmatterService.createBaseFrontmatter(event);
      const markdown = [
        this.obsidianFileService.createFrontmatter(frontmatter),
        "",
        event.content,
  ...
  async getNote(id) {
    ...
    for (const file of files) {
      const content = await this.obsidianFileService.readFile(file);
      const frontmatter = this.obsidianFileService.getFrontmatter(content);
      if ((frontmatter == null ? void 0 : frontmatter.id) === id) {
        const noteContent = ((_a = content.split("\n---\n")[1]) == null ? void 0 : _a.trim()) || "";
...
// src/services/obsidian/follow-file.service.ts
var FollowFileService = class {
  constructor(obsidianFileService, indexService) {
    this.obsidianFileService = obsidianFileService;
    this.indexService = indexService;
    this.followsDir = "nostr/user follows";
  }
  async saveFollow(event) {
    if (event.kind !== 3) {
  ...
  async getFollow(pubkey) {
    var _a;
    const files = await this.indexService.findEventsByKind(3);
    for (const file of files) {
      const content = await this.obsidianFileService.readFile(file);
      const frontmatter = this.obsidianFileService.getFrontmatter(content);
      if (((_a = frontmatter == null ? void 0 : frontmatter.event) == null ? void 0 : _a.pubkey) === pubkey) {
        return frontmatter.event;
      }
    }
...
// src/services/core/timestamp.service.ts
var TimestampService = class {
   ...
   */
  static formatReadableTimestamp(event) {
    const date = new Date(event.created_at * 1e3);
...
// src/services/core/markdown.service.ts
var MarkdownService = class {
   ...
   */
  static generateNoteContent(event, obsidianFileService) {
   ...
   */
  static extractEventData(content, obsidianFileService) {
    var _a;
   ...
   */
  static extractFrontmatter(content, obsidianFileService) {
...
var NostrSettingsTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian4.Setting(containerEl).setName("Public Key").setDesc("Your Nostr public key (npub format)").addText((text) => text.setPlaceholder("npub1...").setValue(this.plugin.settings.pubkey).onChange(async (value) => {
      if (value && !value.startsWith("npub")) {
        new import_obsidian4.Notice("Please enter a valid npub");
        return;
      }
    ...
    this.plugin.settings.relays.forEach((relay, index) => {
      ...
      new import_obsidian4.Setting(relayDiv).addText((text) => text.setPlaceholder("wss://relay.example.com").setValue(relay.url).onChange(async (value) => {
        this.plugin.settings.relays[index].url = value;
        await this.plugin.saveSettings();
      })).addToggle((toggle) => toggle.setValue(relay.enabled).onChange(async (value) => {
    ...
    new import_obsidian4.Setting(containerEl).addButton((button) => button.setButtonText("Add Relay").onClick(async () => {
...
// src/main.ts
var NostrPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    try {
      this.settings = Object.assign({}, DEFAULT_SETTINGS, await this.loadData());
      const enabledRelays = this.settings.relays.filter((r) => r.enabled).map((r) => r.url);
      ...
      this.addCommand({
        ...
        callback: async () => {
          new SearchModal(
            this.app,
            this.vaultService,
            (keyword) => this.noteService.searchNotes(keyword)
          ).open();
        }
      });
      this.addCommand({
        ...
        callback: async () => {
          ...
          if (!npub) {
            new import_obsidian5.Notice("No public key set");
            return;
          }
          try {
            await this.relayService.ensureConnected();
            const hex2 = await this.keyService.npubToHex(npub);
            const event = await this.profileService.fetchEvent(hex2);
            await this.vaultService.saveEvent(event);
            new import_obsidian5.Notice("Profile fetched");
          } catch (error) {
            console.error("Failed to fetch profile:", error);
            new import_obsidian5.Notice("Failed to fetch profile");
          }
        }
      });
      this.addCommand({
        ...
        callback: async () => {
          ...
          try {
            ...
            for (const event of events) {
              await this.vaultService.saveEvent(event);
              await this.linkService.updateLinks(event);
            }
            new import_obsidian5.Notice(`${events.length} notes fetched`);
          } catch (error) {
            console.error("Failed to fetch notes:", error);
            new import_obsidian5.Notice("Failed to fetch notes");
          }
        }
      });
      this.addCommand({
        ...
        callback: async () => {
          ...
          try {
            await this.relayService.ensureConnected();
            const hex2 = await this.keyService.npubToHex(npub);
            const event = await this.followService.fetchEvent(hex2);
            await this.vaultService.saveEvent(event);
            new import_obsidian5.Notice("Follow list fetched");
          } catch (error) {
            console.error("Failed to fetch follow list:", error);
            new import_obsidian5.Notice("Failed to fetch follow list");
          }
        }
      });
      this.addCommand({
        ...
        callback: async () => {
          ...
          try {
            await this.relayService.ensureConnected();
            const hex2 = await this.keyService.npubToHex(npub);
            const followEvent = await this.followService.fetchEvent(hex2);
            const followedPubkeys = followEvent.tags.filter((tag) => tag[0] === "p").map((tag) => tag[1]);
            let fetchedCount = 0;
            for (const pubkey of followedPubkeys) {
              try {
                const profileEvent = await this.profileService.fetchEvent(pubkey);
                await this.vaultService.saveEvent(profileEvent, true);
                fetchedCount++;
              } catch (error) {
                console.error(`Failed to fetch profile for ${pubkey}:`, error);
              }
            }
            new import_obsidian5.Notice(`Fetched ${fetchedCount} follow profiles`);
          } catch (error) {
            console.error("Failed to fetch follow profiles:", error);
            new import_obsidian5.Notice("Failed to fetch follow profiles");
          }
        }
      });
      this.registerEvent(
        this.app.workspace.on("file-menu", (menu, file) => {
          if (file instanceof import_obsidian5.TFile && file.path.startsWith("nostr/user")) {
            menu.addItem((item) => {
              item.setTitle("Fetch Nostr Data").setIcon("sync").onClick(async () => {
                var _a;
                try {
                  const content = await this.app.vault.read(file);
                  const frontmatter = MarkdownService.extractFrontmatter(content, this.obsidianFileService);
                  if (!((_a = frontmatter == null ? void 0 : frontmatter.event) == null ? void 0 : _a.id)) {
                    throw new Error("No event ID found");
                  }
                  await this.relayService.ensureConnected();
                  const event = await this.vaultService.getEvent(frontmatter.event.id);
                  if (event.kind === 0) {
                    const hex2 = await this.keyService.npubToHex(event.pubkey);
                    const profileEvent = await this.profileService.fetchEvent(hex2);
                    await this.vaultService.saveEvent(profileEvent);
                  } else if (event.kind === 1) {
                    await this.noteService.fetchEvent(event.id);
                  } else if (event.kind === 3) {
                    const hex2 = await this.keyService.npubToHex(event.pubkey);
                    await this.followService.fetchEvent(hex2);
                  }
                  new import_obsidian5.Notice("Data fetched");
                } catch (error) {
      ...
      new import_obsidian5.Notice("Nostr plugin initialized");
    } catch (error) {
      console.error("Failed to initialize Nostr plugin:", error);
      new import_obsidian5.Notice("Failed to initialize Nostr plugin");
    }
  }
  onunload() {
