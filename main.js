...
// main.ts
__export(exports, {
  default: () => NostrObsidianGraphPlugin
});
...
function encrypt2(key, plaintext, options = {}) {
  ...
  const salt2 = (_b = options.salt) != null ? _b : randomBytes(32);
...
var NostrObsidianGraphPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.connections = [];
    this.followedProfiles = new Map();
    this.centerHexKey = "";
    this.profileFileMap = new Map();
    this.noteFileMap = new Map();
    this.metadataReceived = new Set();
    this.pendingNotes = false;
    this.followList = [];
    this.notesReceived = new Map();
  }
  async onload() {
    ...
    console.log("Nostr Obsidian Graph plugin loaded!");
  }
  onunload() {
    console.log("Unloading Nostr Graph plugin...");
    this.connections.forEach((socket) => socket.close());
    console.log("Nostr Obsidian Graph plugin unloaded!");
  }
  async loadSettings() {
    console.log("Loading settings...");
    this.settings = Object.assign({}, DEFAULT_SETTINGS, await this.loadData());
    this.settings.notesPerProfile = Math.max(1, Math.min(20, this.settings.notesPerProfile));
  ...
  npubToHex(npub) {
    try {
      ...
      if (npub.startsWith("npub1")) {
        const { data: hexPubkey } = nip19_exports.decode(npub);
        return hexPubkey;
      }
  ...
  sanitizeFilename(name, keepSpaces = false) {
    const sanitized = name.replace(/[^a-z0-9 -]/gi, "").trim();
    return keepSpaces ? sanitized.replace(/\s+/g, " ") : sanitized.replace(/\s+/g, "_");
  }
  getUniqueFilename(baseFilename, existingFiles) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          this.handleNostrEvent(data, url);
        } catch (error) {
  ...
  handleNostrEvent(data, relayUrl) {
    if (!Array.isArray(data))
      return;
      ...
      case "EVENT":
        ...
        if (event.kind === 0) {
          try {
            ...
            this.metadataReceived.add(event.pubkey);
            this.processProfile(event);
          } catch (error) {
            console.error("Failed to parse metadata:", error);
          }
        } else if (event.kind === 1) {
          const notes = this.notesReceived.get(event.pubkey);
          if (notes && !notes.has(event.id)) {
            notes.add(event.id);
            console.log(`New note from ${event.pubkey}, total: ${notes.size}/${this.settings.notesPerProfile}`);
          }
        }
        this.processNostrEvent(event);
        break;
      case "EOSE":
  ...
  async processNostrEvent(event) {
    if (event.pubkey === this.centerHexKey) {
      if (event.kind === 3) {
        await this.processFollows(event);
      }
      return;
    }
    switch (event.kind) {
      case 0:
        await this.processProfile(event);
        break;
      case 1:
  ...
  async processProfile(event) {
    try {
      const metadata = JSON.parse(event.content);
      console.log("Processing profile metadata:", {
        pubkey: event.pubkey,
        name: metadata.name,
        display_name: metadata.display_name,
        nip05: metadata.nip05
      ...
      const profile = this.followedProfiles.get(event.pubkey) || {
        pubkey: event.pubkey,
        notes: []
      };
      if (displayName !== profile.name) {
        console.log(`Updating profile name from "${profile.name}" to "${displayName}"`);
        profile.name = displayName;
        this.followedProfiles.set(event.pubkey, profile);
      }
      console.log("Current profiles:", Array.from(this.followedProfiles.entries()).map(([key, p]) => ({
        pubkey: key,
        name: p.name,
        noteCount: p.notes.length
      })));
    } catch (error) {
      console.error("Failed to process profile:", error, "Raw content:", event.content);
    }
  }
  async processNote(event) {
    try {
      let profile = this.followedProfiles.get(event.pubkey);
      if (!profile) {
        profile = {
          pubkey: event.pubkey,
      ...
      if (!profile.notes.some((n) => n.id === event.id)) {
        profile.notes.push(event);
        profile.notes.sort((a, b) => b.created_at - a.created_at);
        if (profile.notes.length > this.settings.notesPerProfile) {
          profile.notes = profile.notes.slice(0, this.settings.notesPerProfile);
        }
        console.log(`Added note ${event.id} to profile ${profile.name} (total: ${profile.notes.length}/${this.settings.notesPerProfile})`);
      }
    } catch (error) {
      console.error("Failed to process note:", error);
    }
  }
  async processFollows(event) {
    const follows = event.tags.filter((tag) => tag[0] === "p").map((tag) => tag[1]);
    this.followList = follows;
    console.log(`Processing ${follows.length} follows`);
    follows.forEach((pubkey) => {
      this.notesReceived.set(pubkey, new Set());
    });
    for (const relay of this.connections) {
      const metadataSubId = this.generateUUID();
      ...
      console.log("Requesting metadata from relay:", {
        relay: relay.url,
        authors: follows
      });
      relay.send(JSON.stringify(metadataSubscription));
    }
    setTimeout(() => {
      const missingMetadata = follows.filter((f2) => !this.metadataReceived.has(f2));
      console.log("Metadata status:", {
        received: Array.from(this.metadataReceived).length,
        missing: missingMetadata.length,
        missingPubkeys: missingMetadata
      });
      if (missingMetadata.length > 0) {
        for (const relay of this.connections) {
          const retrySubId = this.generateUUID();
          const retrySubscription = [
            "REQ",
            retrySubId,
            {
              "authors": missingMetadata,
              "kinds": [0],
              "limit": 5
            }
          ];
          console.log("Retrying metadata request:", {
            relay: relay.url,
            missing: missingMetadata
          });
          relay.send(JSON.stringify(retrySubscription));
        }
      }
      setTimeout(() => {
        console.log("Final metadata status:", {
          received: Array.from(this.metadataReceived).length,
          missing: follows.filter((f2) => !this.metadataReceived.has(f2)).length,
          profiles: Array.from(this.followedProfiles.entries()).map(([key, p]) => ({
            pubkey: key,
            name: p.name
          }))
        });
        for (const relay of this.connections) {
          const notesSubId = this.generateUUID();
          const notesSubscription = [
            "REQ",
            notesSubId,
            {
              "authors": follows,
          ...
          relay.send(JSON.stringify(notesSubscription));
        }
        setTimeout(() => {
          const noteStatus = Array.from(this.followedProfiles.entries()).map(([key, p]) => {
            var _a;
            return {
              pubkey: key,
              name: p.name,
              noteCount: p.notes.length,
              uniqueNotes: ((_a = this.notesReceived.get(key)) == null ? void 0 : _a.size) || 0
            };
          });
          console.log("Note status:", noteStatus);
          if (!this.pendingNotes) {
            this.pendingNotes = true;
            this.createNostrNotes();
          }
        }, 5e3);
      }, 3e3);
    }, 3e3);
  ...
  async fetchNostrData() {
    console.log("Starting fetchNostrData...");
    if (!this.settings.npub) {
      console.error("No npub configured in settings");
      return;
    }
    try {
      ...
      this.followedProfiles.clear();
      this.connections.forEach((socket) => socket.close());
      this.connections = [];
      this.profileFileMap.clear();
      this.noteFileMap.clear();
      this.metadataReceived.clear();
      this.pendingNotes = false;
      this.followList = [];
      this.notesReceived.clear();
      this.centerHexKey = this.npubToHex(this.settings.npub);
      console.log("Using hex key:", this.centerHexKey);
      for (const relay of this.settings.relays) {
        try {
      ...
      console.error("Error in fetchNostrData:", error);
    }
  }
  async createNostrNotes() {
    console.log("Starting createNostrNotes with profiles:", Array.from(this.followedProfiles.entries()).map(([key, profile]) => {
      var _a;
      return {
        pubkey: key,
        name: profile.name,
    ...
    try {
      await this.app.vault.adapter.mkdir("nostr");
      await this.app.vault.adapter.mkdir("nostr/profiles");
      await this.app.vault.adapter.mkdir("nostr/notes");
      const existingFiles = new Set();
      console.log("Creating My Network note...");
      await this.app.vault.adapter.write("nostr/My Network.md", `---
pubkey: ${this.centerHexKey}
type: nostr-profile
---
...
${Array.from(this.followedProfiles.values()).map((profile) => {
        const displayName = profile.name || profile.pubkey.slice(0, 8);
        console.log(`Creating link for profile: ${displayName}`);
        const safeFilename = this.getUniqueFilename(this.sanitizeFilename(displayName, true), existingFiles);
        this.profileFileMap.set(profile.pubkey, safeFilename);
        existingFiles.add(safeFilename);
        return `- [[nostr/profiles/${safeFilename}|${displayName}]]`;
      }).join("\n")}
`);
      console.log("Creating profile notes...");
      for (const [pubkey, profile] of this.followedProfiles.entries()) {
        const profileFilename = this.profileFileMap.get(pubkey);
        const displayName = profile.name || pubkey.slice(0, 8);
        const noteLinks = await Promise.all(profile.notes.map(async (note) => {
          const truncated = note.content.replace(/\s+/g, " ").trim().substring(0, 50);
          const safeFilename = this.getUniqueFilename(this.sanitizeFilename(truncated, true), existingFiles);
          this.noteFileMap.set(note.id, safeFilename);
          existingFiles.add(safeFilename);
          return `- [[nostr/notes/${safeFilename}|${truncated}]]`;
        }));
        await this.app.vault.adapter.write(`nostr/profiles/${profileFilename}.md`, `---
pubkey: ${pubkey}
name: ${displayName}
type: nostr-profile
aliases: ["${pubkey}"]
...
## Connected From
- [[nostr/My Network|My Network]]
`);
      }
      console.log("Creating note files...");
      for (const profile of this.followedProfiles.values()) {
        for (let i = 0; i < profile.notes.length; i++) {
          const note = profile.notes[i];
          const nextNote = profile.notes[i + 1];
          const prevNote = profile.notes[i - 1];
          const noteFilename = this.noteFileMap.get(note.id);
          await this.app.vault.adapter.write(`nostr/notes/${noteFilename}.md`, `---
id: ${note.id}
pubkey: ${note.pubkey}
date: ${new Date(note.created_at * 1e3).toISOString()}
type: nostr-note
aliases: ["${note.id}"]
---
...
---
## Links
${prevNote ? `- Previous: [[nostr/notes/${this.noteFileMap.get(prevNote.id)}|Previous Note]]` : ""}
${nextNote ? `- Next: [[nostr/notes/${this.noteFileMap.get(nextNote.id)}|Next Note]]` : ""}
`);
        }
      }
      console.log("Finished creating all notes");
    } catch (error) {
      console.error("Failed to create notes:", error);
    }
  }
};
var NostrSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Your npub").setDesc("Your Nostr public key (npub1... format)").addText((text) => text.setPlaceholder("npub1...").setValue(this.plugin.settings.npub).onChange(async (value) => {
      this.plugin.settings.npub = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Notes per Profile").setDesc("Number of notes to fetch per profile (1-20)").addSlider((slider) => slider.setLimits(1, 20, 1).setValue(this.plugin.settings.notesPerProfile).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.notesPerProfile = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Relays").setDesc("List of Nostr relays (one per line)").addTextArea((text) => text.setPlaceholder("wss://relay.example.com").setValue(this.plugin.settings.relays.join("\n")).onChange(async (value) => {
      this.plugin.settings.relays = value.split("\n").map((s) => s.trim()).filter((s) => s.length > 0);
      await this.plugin.saveSettings();
    }));
  }
};
